...
$errorMessage = [];
$amount;
$balance = $accountManager->showBalanceById($_SESSION['user_id']);
if ($_SERVER['REQUEST_METHOD'] === "POST") {
  ...
  $reciverData = $user->getUserByEmail($email);
  if($reciverData === null){
    $errorMessage['emailnotFound'] = "Please enter a valid user email";
  }
  if($_SESSION['user_id'] === $reciverData['user_id']){
    $errorMessage['notPossible'] = "Not Possible to transfer amount same account";
  }
  if($amount > $balance){
    $errorMessage['balanceOVer'] = "Not Possible to transfer More amount over Current Balance";
  }
  if (empty($errorMessage)) {
    $accountManager->transferBalance($amount, $_SESSION['user_id'], $reciverData['user_id']);
  }
}
      ...
      <nav class="border-b border-<?php echo $_SESSION['color'] ;?>-300 border-opacity-25 bg-<?php echo $_SESSION['color'] ;?>-600" x-data="{ mobileMenuOpen: false, userMenuOpen: false }">
            ...
            <div class="flex items-center px-2 lg:px-0">
              <div class="hidden sm:block">
                <div class="flex space-x-4">
                  ...
                  <a href="./withdraw.php" class="text-white hover:bg-<?php echo $_SESSION['color'] ;?>-500 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Withdraw</a>
                  <a href="./transfer.php" class="bg-<?php echo $_SESSION['color'] ;?>-700 text-white rounded-md py-2 px-3 text-sm font-medium">Transfer</a>
                </div>
              </div>
            </div>
                ...
                <div>
                  <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                    ...
                    <!-- <img
                        ...
                        alt="Ahmed Shamim Hasan Shaon" /> -->
                        <span
                        class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-<?php echo $_SESSION['color'] ?>-100">
                        <span class="font-medium leading-none text-<?php echo $_SESSION['color'] ?>-700"
        ...
        <div x-show="mobileMenuOpen" class="sm:hidden" id="mobile-menu">
          <div class="space-y-1 pt-2 pb-3">
            ...
            <a href="./withdraw.php" class="text-white hover:bg-<?php echo $_SESSION['color'] ;?>-500 hover:bg-opacity-75 block rounded-md py-2 px-3 text-base font-medium">Withdraw</a>

            <a href="./transfer.php" class="bg-<?php echo $_SESSION['color'] ;?>-700 text-white block rounded-md py-2 px-3 text-base font-medium">Transfer</a>
          </div>
          <div class="border-t border-<?php echo $_SESSION['color'] ;?>-700 pb-3 pt-4">
              <div class="flex items-center px-5">
                ...
                <button
                  type="button"
                  class="ml-auto flex-shrink-0 rounded-full bg-<?php echo $_SESSION['color'] ;?>-600 p-1 text-<?php echo $_SESSION['color'] ;?>-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-<?php echo $_SESSION['color'] ;?>-600">
              ...
              <div class="mt-3 space-y-1 px-2">
                <a
                  href="/logout.php"
                  class="block rounded-md px-3 py-2 text-base font-medium text-white hover:bg-<?php echo $_SESSION['color'] ;?>-500 hover:bg-opacity-75"
                  >Sign out</a
                >
              </div>
            </div>
      ...
      <div class="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg p-2">
          ...
          <div class="sm:rounded-lg">
            ...
            <?php if (isset($errorMessage['notPossible'])) : ?>
          <div class="text-xs text-red-700 pt-2">
            <?php echo $errorMessage['notPossible'] ?>
          </div>
          <?php endif; ?>
            <?php if (isset($errorMessage['balanceOVer'])) : ?>
          <div class="text-xs text-red-700 pt-2">
            <?php echo $errorMessage['balanceOVer'] ?>
          </div>
        <?php endif; ?>
              <div class="mt-4 text-sm text-gray-500">
                <form action="transfer.php" method="POST">
                  ...
                  <div class="relative mt-4 md:mt-8">
                    ...
                    <input type="number" name="amount" id="amount" class="block w-full ring-0 outline-none pl-4 py-2 md:pl-8 text-gray-800 border-b border-b-<?php echo $_SESSION['color'] ;?>-500 placeholder:text-gray-400 md:text-4xl" placeholder="0.00" required />
                  </div>

                  <!-- Submit Button -->
                  <div class="mt-5">
                    <button type="submit" class="w-full px-6 py-3.5 text-base font-medium text-white bg-<?php echo $_SESSION['color'] ;?>-600 hover:bg-<?php echo $_SESSION['color'] ;?>-800 focus:ring-4 focus:outline-none focus:ring-<?php echo $_SESSION['color'] ;?>-300 rounded-lg md:text-xl text-center">
                      Proceed
                    </button>
                  </div>
                </form>
