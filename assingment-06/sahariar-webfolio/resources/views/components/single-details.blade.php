@props(['project'])
<section class="bg-sd-bg">
<div class="max-w-3xl px-4 pt-6 lg:pt-10 pb-12 sm:px-6 lg:px-8 mx-auto">
    <div class="max-w-2xl">
      <!-- Content -->
      <div class="space-y-5 md:space-y-8 py-10">
        <div class="space-y-3 py-10">
          <h2 class="text-2xl font-bold md:text-4xl dark:text-white text-center">{{ $project['title'] }}</h2>
        </div>
        <figure>
            <img class="w-full object-cover rounded-xl" src="{{ asset($project['image']) }}" alt={{ $project['title'] }}>
            <figcaption class="mt-3 text-sm text-center text-gray-500 dark:text-neutral-500">
              A woman sitting at a table.
            </figcaption>
          </figure>
        <p class="text-lg text-gray-800 dark:text-neutral-200">{{$project['description']}}</p>

        <div class="py-6">
          <div class="flex flex-wrap items-center">
            <p class="text-white"><strong>Technologies: </strong></p>
            @foreach ($project['technologies'] as $technology)
                <a class="m-1 inline-flex items-center gap-1.5 py-2 px-3 rounded-full text-sm text-gray-800 bg-pm-dev hover:bg-gray-200 focus:outline-none focus:bg-gray-200" href="#">
                    {{ $technology }}
                </a>
            @endforeach
        </div>
